...
// --- GROQ AI API INTEGRATION HANDLER ---
// මේ කොටස අලුතින් එකතු කළා AI එක වැඩ කරන්න
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['action']) && $_GET['action'] === 'chat') {
    ...
    $apiKey = "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"; // ඔයා දුන්න Groq API Key එක
    
    // Prompt එක භාෂාව අනුව හැඩගැස්වීම
    $systemPrompt = "You are a helpful academic assistant for FCBS Digi Kuppiya. Answer in $lang language clearly.";
    ...
    exit; // API request එකක් නිසා මෙතනින් නවත්වනවා
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
        ...
        if (isset($_FILES['profilePic']) && $_FILES['profilePic']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($ext, ['jpg', 'jpeg', 'png'])) {
                ...
                if ($stmt->execute()) {
                    ...
                    header("Location: " . $_SERVER['PHP_SELF'] . "?success=pic"); exit;
                }
            }
        }
    }
    if (isset($_POST['updateDetails'])) {
        ...
        if ($stmt->execute()) {
            $res = $conn->query("SELECT * FROM users WHERE id=" . $user['id']);
            $_SESSION['user'] = $res->fetch_assoc();
            header("Location: " . $_SERVER['PHP_SELF'] . "?success=details"); exit;
        }
    }
}
...
$subjects = [
    ['title' => 'Principle of Management', 'link' => 'pom.php', 'icon' => 'briefcase', 'color' => 'from-blue-500 to-cyan-400', 'bg' => 'bg-blue-500/10', 'border' => 'border-blue-500/20', 'text' => 'text-blue-400'],
    ['title' => 'Mathematics', 'link' => 'math.php', 'icon' => 'calculator', 'color' => 'from-emerald-500 to-teal-400', 'bg' => 'bg-emerald-500/10', 'border' => 'border-emerald-500/20', 'text' => 'text-emerald-400'],
    ['title' => 'Information Technology', 'link' => 'ict.php', 'icon' => 'cpu', 'color' => 'from-violet-500 to-purple-400', 'bg' => 'bg-violet-500/10', 'border' => 'border-violet-500/20', 'text' => 'text-violet-400'],
    ['title' => 'Micro Economics', 'link' => 'micro economics.php', 'icon' => 'trending-up', 'color' => 'from-orange-500 to-yellow-400', 'bg' => 'bg-orange-500/10', 'border' => 'border-orange-500/20', 'text' => 'text-orange-400'],
    ['title' => 'Basic English I', 'link' => 'english1.php', 'icon' => 'languages', 'color' => 'from-rose-500 to-pink-400', 'bg' => 'bg-rose-500/10', 'border' => 'border-rose-500/20', 'text' => 'text-rose-400'],
    ['title' => 'Basic Tamil I / Sinhala I', 'link' => '', 'icon' => 'pen-tool', 'color' => 'from-amber-500 to-orange-400', 'bg' => 'bg-amber-500/10', 'border' => 'border-amber-500/20', 'text' => 'text-amber-400']
];
?>
...
<header class="sticky top-0 z-50 w-full px-6 py-4 flex items-center justify-between backdrop-blur-md bg-white/5 border-b border-white/10">
    <div class="flex items-center gap-4">
        <img src="assets/img/logo.png" alt="Logo" class="h-14 w-auto object-contain">
    ...
    <div class="relative">
        ...
        <div x-show="profileDropdown" @click.away="profileDropdown = false" class="absolute right-0 mt-3 w-56 bg-slate-900 rounded-2xl shadow-2xl py-2 border border-white/10">
            ...
            <button @click="modalOpen = true; profileDropdown = false" class="w-full text-left px-4 py-3 text-sm text-gray-300 hover:bg-white/10 flex items-center gap-2">
                <i data-lucide="user-cog" class="w-4 h-4 text-yellow-300"></i> Edit Account
            </button>
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end mb-12 gap-6">
        <div>
            <div class="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/10 border border-white/20 text-white mb-6">
                ...
                <span class="font-medium text-xs uppercase tracking-widest">Year I Semester I</span>
            </div>
            <h1 class="text-5xl md:text-7xl font-bold text-white tracking-tight leading-tight">
                FCBS Digi Kuppiya <br />
                <span class="text-transparent bg-clip-text bg-gradient-to-r from-yellow-300 to-pink-300">1.1 Starts Here</span>
            </h1>
        </div>
    ...
    <div id="chatWindow" class="hidden mb-4 w-[calc(100vw-3rem)] sm:w-[380px] overflow-hidden rounded-3xl border border-white/10 bg-slate-900 shadow-2xl">
        <div class="bg-gradient-to-r from-blue-600 to-purple-600 p-5">
            ...
            <div class="flex gap-2">
                ...
                <button onclick="setLanguage('si', this)" class="lang-btn flex-1 bg-white/10 py-2 rounded-lg text-[10px] font-bold border border-white/10">සිංහල</button>
                <button onclick="setLanguage('ta', this)" class="lang-btn flex-1 bg-white/10 py-2 rounded-lg text-[10px] font-bold border border-white/10">தமிழ்</button>
            </div>
        </div>
        <div class="h-80 p-4 overflow-y-auto space-y-4 flex flex-col" id="msgLog">
            <div class="bg-white/5 p-3 rounded-2xl text-xs text-white/70 max-w-[80%] border border-white/10">Hello! I'm your FCBS Assistant. How can I help you today?</div>
        </div>
        <div class="p-4 border-t border-white/5 flex gap-2">
            ...
            <button id="chatSendBtn" onclick="sendChat()" class="bg-blue-600 p-2 rounded-xl text-white hover:bg-blue-500 transition-colors"><i data-lucide="send" class="w-5 h-5"></i></button>
        </div>
    </div>
...
<script>
    ...
    function appendMessage(text, isUser = false) {
        ...
        if (isUser) {
            msgDiv.className = "bg-blue-600/20 p-3 rounded-2xl text-xs text-white self-end ml-auto max-w-[80%] border border-blue-500/30";
        } else {
            msgDiv.className = "bg-white/5 p-3 rounded-2xl text-xs text-white/70 mr-auto max-w-[80%] border border-white/10";
        }
    ...
    async function sendChat() {
        ...
        typingDiv.className = "bg-white/5 p-3 rounded-2xl text-xs text-white/40 italic mr-auto";
        typingDiv.innerHTML = 'Thinking<span class="loading-dots">...</span>';
        msgLog.appendChild(typingDiv);
        msgLog.scrollTop = msgLog.scrollHeight;

        try {
            // Groq API එකට Backend හරහා Request එක යවනවා

...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['updatePic'])) {
    if (isset($_FILES['profilePic']) && $_FILES['profilePic']['error'] === UPLOAD_ERR_OK) {
        ...
        if (in_array($ext, $allowed_ext) && $_FILES['profilePic']['size'] <= 3 * 1024 * 1024) {
            if ($user['profilePic'] && file_exists('../' . $user['profilePic']) && basename($user['profilePic']) != 'default.png') {
...
/**
 * ResourceCard function for Micro Economics
 */
...
?>
    <a href="<?= $link ?>" class="resource-card group relative overflow-hidden rounded-3xl border p-8 min-h-[210px] flex flex-col justify-between transition-all duration-300 hover:shadow-2xl hover:-translate-y-2 bg-white/5 <?= $styleClass ?> no-underline">
        ...
        <div class="mt-6 text-sm font-semibold text-white flex items-center">
...
<head>
    ...
    <title>Micro Economics - Digi Kuppiya</title>
...
<body class="text-white" x-data="{ modalOpen: <?= (isset($_GET['success']) ? 'true' : 'false') ?>, activeTab: 'details', profileDropdown: false }">
    ...
    <header class="sticky top-0 z-40 w-full border-b border-white/10 bg-white/5 backdrop-blur-md">
        <div class="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            ...
            <div class="relative">
                ...
                <div x-show="profileDropdown" @click.away="profileDropdown = false" class="absolute right-0 mt-3 w-56 bg-slate-900 rounded-2xl shadow-2xl py-2 border border-white/10 overflow-hidden" x-transition>
                    ...
                    <a href="../logout.php" class="block px-4 py-3 text-sm text-red-400 hover:bg-red-500/10 border-t border-white/10 flex items-center gap-2 no-underline">
    ...
    <main class="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
        <div class="rounded-[2.5rem] bg-white/5 p-10 border border-white/10 relative overflow-hidden mb-12">
            ...
            <div class="relative z-10">
                ...
                <h1 class="text-4xl font-black text-white sm:text-6xl mb-2">Micro Economics</h1>
                <div class="flex flex-wrap items-center gap-4 mt-4 text-white/60">
                    <div class="flex items-center gap-2">
                         <i data-lucide="calendar" class="h-5 w-5 text-blue-400"></i>
                         <span>Year I • Semester I</span>
                    </div>
                    <div class="hidden sm:block h-1 w-1 rounded-full bg-white/20"></div>
                    <div class="flex items-center gap-2">
                         <i data-lucide="graduation-cap" class="h-5 w-5 text-emerald-400"></i>
        ...
        <div class="grid grid-cols-1 gap-8 sm:grid-cols-2 lg:grid-cols-4">
            <?php
            renderResourceCard("Chapters", "09", "book-open", "blue", "micro economics chapters.php");
            renderResourceCard("Past Papers", "04", "history", "purple", "micro economics pp.php");
            renderResourceCard("Short Notes", "02", "clipboard-list", "emerald", "micro economics short.php");
            renderResourceCard("Kuppi Videos", "03", "play-circle", "orange", "micro economics video.php");
            ?>
        </div>
    </main>
        ...
        <div class="modal-content shadow-2xl">
            ...
            <div x-show="activeTab === 'details'">
                <form method="post" class="space-y-4">
                    <div class="space-y-1">
                        <label class="text-[10px] uppercase tracking-wider text-gray-500 font-bold ml-1">First Name</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] uppercase tracking-wider text-gray-500 font-bold ml-1">Last Name</label>
                    ...
                    <div class="space-y-1">
                        <label class="text-[10px] uppercase tracking-wider text-gray-500 font-bold ml-1">Mobile Number</label>
                    ...
                    <button type="submit" name="updateDetails" class="w-full bg-gradient-to-r from-yellow-300 to-orange-400 text-slate-900 font-bold py-3.5 rounded-xl mt-2 transition-transform active:scale-95">Update Information</button>
                </form>
            </div>
            
            <div x-show="activeTab === 'picture'" class="text-center">
                <img src="<?= $profilePicPath; ?>" class="w-24 h-24 rounded-2xl object-cover ring-2 ring-yellow-300/20 mx-auto mb-6">
                <form method="post" enctype="multipart/form-data">
                    ...
                    <label for="file-upload" class="cursor-pointer inline-block bg-white/10 border border-white/10 px-8 py-3 rounded-xl text-sm font-bold hover:bg-white/20">Change Photo</label>
                </form>
            </div>
        </div>
    </div>
